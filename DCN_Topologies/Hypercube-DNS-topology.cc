...
NS_LOG_COMPONENT_DEFINE ("HyperCube-DNS-Architecture");




void hyper_Throughput(double throughput)
{
  std::string fileNameWithNoExtension = "HyperCube_throughput";
...
throughput=throughput/100.0;
  for (x = 0.001; x < throughput; x += 0.01)
    {
      y = x+0.012 ;
      dataset.Add (x, y);
    }
...
int main (int argc, char *argv[])
{
  ...
  std::string tr_name ("HyperCube16node.tr");
  std::string pcap_name ("HyperCube16node");
  std::string flow_name ("HyperCube16node.xml");
  std::string anim_name ("HyperCube16node.anim.xml");

  std::string adj_mat_file_name ("hyper.txt");
